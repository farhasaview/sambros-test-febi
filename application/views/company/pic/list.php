...
<?php
  if (!empty($all)) {
    if (count($all) == 3) { ?>
  <button type="button" disabled><i class="fa fa-lock"></i> <b>Maksimal Akun PIC adalah 3</b></button>
<?php }else{ ?>
  ...
  <table class="table table-bordered table-striped table-responsive bg-light">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>
          <a onclick="return confirm('apakah anda yakin?')" href="<?= base_url()?>company/deletePic/<?=$all->pic_id?>" type="button" class="btn btn-outline-danger btn-sm"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
        </td>
      </tr>
     <?php }?>
    </tbody>
